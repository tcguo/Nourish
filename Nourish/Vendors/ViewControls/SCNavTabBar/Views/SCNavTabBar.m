...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        [button setTitleColor:ColorBaseFont forState:UIControlStateNormal];
...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles
{
    ...
    CGFloat totalFontWidth = 0;
    for (NSString *title in titles)
    {
        CGSize size = [title sizeWithAttributes:attributes];
        totalFontWidth += size.width;
    }
    
    CGFloat padding = 40;
    if (self.bounds.size.width > totalFontWidth) {
        padding = (self.bounds.size.width-totalFontWidth)/(titles.count);
    }

    for (NSString *title in titles)
    {
        CGSize size = [title sizeWithAttributes:attributes];
        NSNumber *width = [NSNumber numberWithFloat:size.width + padding];
