...
//    [self addSubview:leftLine];
    ...
    switch (type) {
        case MozAlertTypeInfo:{
            ...
            self.backgroundColor = RgbHex2UIColorWithAlpha(0XFF, 0XFF, 0XFF, 0.12);
            leftIcon.hidden = YES;
...
//        [UIView animateWithDuration:1.5 delay:0.0 options:<#(UIViewAnimationOptions)#> animations:^{
//             self.layer.position = CGPointMake(self.layer.position.x, self.layer.position.y + 40);
//        } completion:^(BOOL finished) {
//            
//        }];
        
        [UIView animateWithDuration:2.0 delay:0 usingSpringWithDamping:1.0 initialSpringVelocity:0 options:UIViewAnimationOptionCurveLinear animations:^{
            self.layer.position = CGPointMake(self.layer.position.x, self.layer.position.y + 40);
        } completion:^(BOOL finished) {
