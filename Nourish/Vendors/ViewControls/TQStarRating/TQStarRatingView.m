...
- (id)initWithFrame:(CGRect)frame numberOfStar:(int)number
{
    ...
    if (self) {
        ...
        self.starForegroundView = [self buidlStarViewWithImageName:@"comment-star-selected"];
         self.starBackgroundView = [self buidlStarViewWithImageName:@"comment-star-normal"];
...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    for (int i = 0; i < self.numberOfStar; i ++)
    {
        ...
        imageView.frame = CGRectMake(x, (CGRectGetHeight(self.frame)-24)/2, 24, 22);
...
- (void)changeStarForegroundViewWithPoint:(CGPoint)point
{
    ...
    self.starBackgroundView.frame = CGRectMake(0, 0, p.x, self.frame.size.height);
